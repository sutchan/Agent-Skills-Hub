...
展示页为纯静态 SSG，`site/data/skills.json` 是唯一数据接口，由 `site/build_site.py` 在 `prebuild` 阶段从磁盘生成。**严禁手改业务字段**，仅允许本地修正后重跑 `npm run build`。
...
| 输出 | 写入 `site/data/skills.json`，UTF-8，缩进 2 |
...
5. **展示页规范**：UI 改动须对齐 `site/DESIGN.md`（配色令牌、响应式、可访问性、交互流程）。
6. **数据契约**：`skills.json` 的结构、字段语义与生成规则以 §4.5 为准；新增/修改技能字段须同步更新 `build_site.py`、`site/DESIGN.md` 与 §4.5，三者严格对应、不得歧义。
