...
> 路径：`openspec/project.md` · 版本：1.14.48
...
```jsonc
{
  "total":      整数,                         // 技能总数（动态统计 = skills.length，当前 173）
...
| 技能数据来源 | 权威数据 = `skills/<name>/SKILL.md` 正文（`build-skills-data.mjs` 解析正文提取英文描述；frontmatter 为可选字段，当前技能均未使用），`build-skills-data.mjs` 生成 `data/skills-data.json`，`build.mjs` 已将全部技能（当前 173 / 13 分类）预渲染进 `prototype/index.html` |
...
| 计数 | `total` = 技能总数（动态统计，当前 173）；各分类 `count` 由脚本统计写入；README 声明 173 与 `data/skills-data.json` 一致 |
