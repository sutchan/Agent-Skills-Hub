...
name: video
description: |-
  当用户要用 AI 工具或程序化框架创建、生成或制作视频内容时触发。涵盖「视频制作」「AI 视频」「Remotion」「Hyperframes」「HeyGen」「Synthesia」「Veo」「Sora」「Runway」「Kling」「Seedance」「Hailuo」「MiniMax」「Pika」「Hunyuan」「Wan」「视频生成」「AI 数字人」「口播视频」「程序化视频」「视频模板」「讲解视频」「产品演示视频」「视频流水线」「复制这个剪辑」「匹配这种视频风格」「逆向这个视频」「像参考片那样剪辑」「给我做个视频」。用于视频创作、生成与生产工作流。视频内容策略与选题见 social，付费视频广告创意见 ad-creative。
metadata:
  version: 2.1.0
