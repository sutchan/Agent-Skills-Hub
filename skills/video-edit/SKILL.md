...
name: video-edit
description: |-
  在 RunComfy 上编辑现有视频——本技能是智能路由器，将用户意图匹配到 RunComfy 目录中合适的编辑模型。通用重风格/换背景/换包装选 Wan 2.7 Edit-Video，精确运动迁移选 Kling 2.6 Pro Motion Control，轻量身份稳定重风格/换装选 Lucy Edit Restyle。附带各模型提示词范式，通过本地 RunComfy CLI 调用。
