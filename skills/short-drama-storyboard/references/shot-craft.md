...
已接受的原文落实表要保存完整的 `shot_refs`，不能只写裸镜头 ID。文件的 `sources` 为每个
上游快照声明一次负责人与准确文件，每条引用再用 `src` 指向这条声明并
写上 `record_id`，这样才能知道审查的是哪一版。同一文件内部的关系可以用 `*_ids`；
跨文件关系必须用 `*_refs`，以便来源改变后发现过期引用。
