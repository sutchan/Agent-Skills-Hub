...
可渲染检查”照旧逐条写满，本词表只负责画风投影、光色倾向和当前帧的可读性约束。
事实先于审美词的优先级不变——词表里的任何词都不能顶替身份、地理、构图与连续性事实。

## 规则分级与语言政策

- 本词表中的画风与镜头级选词都是 `taste_option`：它们供比较，不是质量门槛。某个方向被创作者接受并
  写入 `short-drama.json#/creator_authority/visual_direction/choices/look_development` 后，
  才成为本项目必须投影的视觉选择；在那之前，审查不得只因“没用词表里的词”判为问题。
...
2. **视觉语言投影**：项目级共享画风基底 + 当前镜头需要的光色或表现手段。先从已接受的
   `look_development` 读取稳定项，再按镜头目的补局部变化；不能每集回表重新选一套。
3. **可读性 / 风险约束**：只写当前帧确实可能失去的可见信息，例如目光、持物、特效起点或
   前中后景层级。分辨率、供应商质量标签和固定负面词尾不是这一层。

## 项目级共享画风基底

画风基底属于项目，不属于某一种镜头。分镜阶段从已接受的
`visual_direction.choices.look_development` 读取“选择 + 可观察写法”；尚未接受时回到 Look
Development 比较和记录，不在单镜里代选。叙事、对话和动作帧都读取同一条项目记录。
...
2. 读取已接受的 `visual_direction.choices.look_development`。若项目级基底仍只有名称，或尚未
   记录，先回到 `$short-drama` 完成 Look Development、发布并接受创作者决策，再用
   `set-authority` 写回；镜头级局部选词留在当前关键帧，不写回项目基底。
...
1. **项目级画风漂移**：同一项目临时从“赛璐璐动画帧”换到“厚涂漫画”，或只记名称、不记
   可观察投影。项目基底应由创作者接受并写入 `look_development`；单镜只使用已允许的局部变化。
